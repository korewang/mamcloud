...
(function($){
  ...
  var proxyHandlers = {
        ...
        if ($target) {
          ...
          if (touches.length === settings.requiredTouches) {
            switch (eventType) {
              ...
              //////////////// LONGPRESS ////////////////
              case 'longpress':
                   data.lastMovePoint = {"x":touches[0].pageX,"y":touches[0].pageY};
                   var distanceXY =  {"x":Math.abs(data.lastMovePoint.x - data.startPoint.x),"y":Math.abs(data.lastMovePoint.y - data.startPoint.y)};
                   if(distanceXY.x > 30 || distanceXY.y > 30){
      ...
      handleTouchEnd: function (e) {
        var eventType = this.context,
            $target = getTarget(e, eventType);
        //originalEvent. changedTouches[0]  pageX  pageY
        if ($target) {
          var event = e.originalEvent,
              camelDataName = 'touchy' + eventType.charAt(0).toUpperCase() + eventType.slice(1),
              data = $target.data(camelDataName),
              settings = data.settings,
              touchEnds = event.changedTouches,
              endX = touchEnds[0].pageX,
              endY = touchEnds[0].pageY,
              moveX = Math.abs(endX - data.startPoint.x),
              moveY = Math.abs(endY - data.startPoint.y);
